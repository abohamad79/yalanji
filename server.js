...
function loadStore() {
  ...
  if (fs.existsSync(FILE)) {
    try {
      ...
      console.error('ملف البيانات تالف — نسخة احتياطية باسم store.bad.json');
      fs.renameSync(FILE, path.join(DATA, 'store.bad.json'));
      store = {};
    }
  }
  let seeded = false;
...
function flush() {
  ...
  fs.renameSync(tmp, FILE);    // كتابة ذرّية — لا يتلف الملف عند انقطاع الكهرباء
}
function saveStore() {
  clearTimeout(saveTimer);
  saveTimer = setTimeout(flush, 120);
}
// لا يضيع أي تعديل عند إيقاف الخادم أو إعادة تشغيله
['SIGINT', 'SIGTERM', 'SIGHUP'].forEach((sig) =>
  process.on(sig, () => { try { if (saveTimer) flush(); } catch (e) {} process.exit(0); }));
process.on('exit', () => { try { if (saveTimer) flush(); } catch (e) {} });
  ...
  try {
    ...
    if (seg[0] === 'api' && seg[1]) {
      ...
      if (act === 'state' && req.method === 'PUT') {
        if (!sess) return fail(res, 401, 'الجلسة منتهية');
        const b = await readBody(req);
        store[slug] = { ...sanitize(b, d), pw: d.pw };
        saveStore();
        publish(slug);
